...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        newUser.appendChild(document.createTextNode(data.user_joined));
        newUser.className = data.user_joined;
        newUser.setAttribute('aria-live', 'polite');
        newUser.setAttribute('aria-label', 'new user joined');
        console.log(newUser);
        document.querySelector("#users-list>ul").appendChild(newUser);

    } else if (data.type === "disconnect") {
        console.log('A user has just disconnected:');
        console.log(data);

        exit = document.querySelector(`.${data.user_left}`)
        exit.setAttribute('aria-live', 'polite');
        exit.setAttribute('aria-label', 'User '+ data.user_left +' just left');
        exit.remove();
        ...
        htmlObject.innerHTML = `${data.username}: ${data.text}`
        htmlObject.setAttribute('aria-live', 'polite');

        chat_box = document.querySelector("#chat");
        if (data.username === username) {
            // display right
            console.log(htmlObject);
            // console.log(htmlObject.classList);
            htmlObject.classList += "right"
            htmlObject.setAttribute('aria-label', "Sent new message");
        } else {
            //display left
            htmlObject.classList += "left"
            htmlObject.setAttribute('aria-label', "Received new message");
        };
        chat_box.appendChild(htmlObject);
